...
class gdEncoder(nn.Module):
    def __init__(self, num_layers, nfeat, nhid, nclass, heads, negative_slope, concat, dropout):
        super(gdEncoder, self).__init__()
        self.body = gdEncoder_body(num_layers, nfeat, nhid, heads, negative_slope, concat, dropout)
...
class gdEncoder_body(nn.Module):
    def __init__(self, num_layers, nfeat, nhid, heads, negative_slope, concat, dropout):
        super(gdEncoder_body, self).__init__()
        ...
        self.layers.append(SimAttConv(nfeat, nhid, heads, self.concat, self.negative_slope, self.dropout))
        # hidden layers
        for l in range(1, num_layers):
            # due to multi-head, the nfeat = nhid * num_heads
            if self.concat == True:
                self.layers.append(SimAttConv(nhid * heads, nhid, heads, self.concat, self.negative_slope))
            else:
                self.layers.append(SimAttConv(nhid, nhid, heads, self.concat, self.negative_slope))
