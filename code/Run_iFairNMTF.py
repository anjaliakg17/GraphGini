...
parser.add_argument('--alpha', type=float, default=0.0000001,
...
parser.add_argument('--model', type=str, default='jk',
                    choices=['gcn', 'gin', 'jk'])
...
RS = compute_RS(labels)
RD = compute_RD(labels)
...
for epoch in range(args.initialize_training_epochs+1):
    ...
    ifair_loss2 = torch.trace( torch.mm( output.t(), torch.sparse.mm(lap2, output) ) )

    #loss_label_init_train.backward()
    loss_guide_train = loss_label_init_train + args.alpha * ifair_loss2
...
#     model.train()
#     optimizer.zero_grad()
#     ifgOutput = model(features, edge_index)
...
#     ifair_loss2 = torch.trace( torch.mm( ifgOutput.t(), torch.sparse.mm(lap2, ifgOutput) ) )
...
#     loss_guide_train = loss_label_guide_train + args.alpha * ifair_loss2
#     loss_guide_train.backward()
#     optimizer.step()
