#this code has been taken from GUIDE repo
import os
import torch
import random
import numpy as np
...
###################################################################################################
def calculate_similarity_matrix(adj, features, metric=None, filterSigma = None, normalize = None):
...
def convert_sparse_matrix_to_sparse_tensor(X):
    X = X.tocoo()
    X = torch.sparse_coo_tensor(torch.tensor([X.row.tolist(), X.col.tolist()]), torch.tensor(X.data.astype(np.float32)))
    return X
...
def filter_similarity_matrix(sim, sigma):
    ...
    """
    print("***********filter_similarity_matrix")
