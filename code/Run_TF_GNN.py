...
for epoch in range(args.initialize_training_epochs+1):
    ...
    ifair_loss = torch.trace( torch.mm( output.t(), torch.sparse.mm(lap, output) ) )

    #loss_label_init_train.backward()
    loss_guide_train = loss_label_init_train + args.alpha * ifair_loss
